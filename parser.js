const parsers = [
  ...
  { bank: 'Bank Of America', regex: /Amount: \$(\d+.\d+)[\s\S]*Where: ([\s\S]*)\nType:[\s\S]*Transaction date: (\w+) (\d+), (\d{4})/, amount_group: 1, payee_group: 2, month_group: 3, day_group: 4, year_group: 5 },
  
  // Wells Fargo
  // Example:
  //   *Credit card* XXXX-XXXX-XXXX-1111\n\nPurchase amount* 24.23 USD\n\n*Purchase location* at TRUE VALUE #0023 in DESTIN USA\n\n*Date* 08/30/2018
  { bank: 'Wells Fargo', regex: /Purchase amount\* (\d+.\d+)[\s\S]*Purchase location\* ([\s\S]*)Date\* (\d{2})\/(\d{2})\/(\d{4})/, amount_group: 1, payee_group: 2, month_group: 3, day_group: 4, year_group: 5 },
  ...
  { bank: 'PayPal', regex: /You sent \$?(\d+.\d+)[\s\S]*to ([\s\w]*)Transaction Details[\s\S]*Transaction ID: [\s\S]* (\w+) (\d{2}), (\d{4})/, amount_group: 1, payee_group: 2, month_group: 3, day_group: 4, year_group: 5 }
];
